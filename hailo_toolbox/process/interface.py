...
class DataContainer(Generic[T]):
    ...
    def get_current_data(self) -> Any:
        ...
        if (
            ...
            and self.visualization_data is not None
        ):
            return self.visualization_data
        elif (
            stage == ProcessingStage.POSTPROCESSING
            and self.postprocessed_results is not None
        ):
            ...
            and self.preprocessed_data is not None
        ):
...
class DataInterface(ABC, Generic[InputType, OutputType]):
    ...
    @abstractmethod
    def process(
        self, input_data: DataContainer[InputType]
    ) -> DataContainer[OutputType]:
        """
...
def create_image_container(
    image: np.ndarray, source_path: str = None, metadata: Metadata = None
...
def create_batch_container(
    batch: List[np.ndarray], metadata: Metadata = None
) -> DataContainer[List[np.ndarray]]:
...
def create_tensor_container(
    tensor: np.ndarray, metadata: Metadata = None
) -> DataContainer[np.ndarray]:
