...
from .preprocessor.preprocessor import PreprocessConfig
...
class MockInferenceEngine:
    ...
    def infer(self, input_tensor: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        if self.task_type == "detection":
            ...
            detection_output[:, :, 0] = np.random.uniform(
                0.1, 0.9, (batch_size, num_detections)
            )  # x
            detection_output[:, :, 1] = np.random.uniform(
                0.1, 0.9, (batch_size, num_detections)
            )  # y
            detection_output[:, :, 2] = np.random.uniform(
                0.05, 0.3, (batch_size, num_detections)
            )  # w
            detection_output[:, :, 3] = np.random.uniform(
                0.05, 0.3, (batch_size, num_detections)
            )  # h
            ...
            mask_prototypes = np.random.randn(
                batch_size, num_mask_coeffs, proto_h, proto_w
            )
...
def example_detection_pipeline():
    ...
    try:
        ...
        config = PipelineConfig(
            ...
            output_config={
                "path": temp_image_path.replace(".jpg", "_output.jpg"),
                "format": "image",
            },
        )
        ...
        preprocessor = PreprocessorAdapter(
            PreprocessConfig(target_size=(640, 640), normalize=True)
        )
        inference_engine = MockInferenceEngine("detection", 80)
        inference = InferenceAdapter(
            inference_engine,
            {"name": "YOLOv8-Detection", "version": "1.0", "task": "detection"},
        )
        postprocessor = PostprocessorAdapter(
            "detection",
            PostprocessConfig(
                num_classes=80, det_conf_threshold=0.25, nms_iou_threshold=0.5
            ),
        )
        ...
        pipeline.add_component(source).add_component(preprocessor).add_component(
            inference
        ).add_component(postprocessor).add_component(visualization).add_component(
            output
        )
        ...
        print(
            f"Processing stages: {[stage.value for stage in pipeline.components.keys()]}"
        )
        ...
        if result.postprocessed_results is not None:
            ...
            if len(detection_results) > 0:
                ...
                print(
                    f"Confidence scores: {detection_results.scores[:5]}"
                )  # First 5 scores
                print(
                    f"Class IDs: {detection_results.class_ids[:5]}"
                )  # First 5 class IDs
...
def example_segmentation_pipeline():
    ...
    try:
        ...
        # Create pipeline configuration
        config = PipelineConfig(enable_profiling=True, continue_on_error=False)
        ...
        inference = InferenceAdapter(
            inference_engine,
            {"name": "YOLOv8-Segmentation", "version": "1.0", "task": "segmentation"},
        )
        postprocessor = PostprocessorAdapter(
            "segmentation",
            PostprocessConfig(
        ...
        pipeline.add_component(source).add_component(preprocessor).add_component(
            inference
        ).add_component(postprocessor).add_component(visualization)
        ...
        if result.postprocessed_results is not None:
            ...
            if len(seg_results) > 0:
                ...
                for i in range(min(3, len(seg_results))):
                    mask_coverage = (
                        np.sum(seg_results.masks[i]) / seg_results.masks[i].size * 100
                    )
...
def example_keypoint_pipeline():
    ...
    try:
        ...
        inference = InferenceAdapter(
            inference_engine,
            {"name": "YOLOv8-Pose", "version": "1.0", "task": "keypoint"},
        )
        postprocessor = PostprocessorAdapter(
            "keypoint",
            PostprocessConfig(
    ...
    try:
        ...
        for i in range(3):
            with tempfile.NamedTemporaryFile(
                ...
                cv2.imwrite(
                    temp_image_path, cv2.cvtColor(mock_image, cv2.COLOR_RGB2BGR)
                )
        ...
        inference = InferenceAdapter(
            inference_engine,
            {"name": "YOLOv8-Detection-Batch", "version": "1.0", "task": "detection"},
        )
        postprocessor = PostprocessorAdapter(
            "detection", PostprocessConfig(num_classes=80, det_conf_threshold=0.25)
        )
        ...
        total_detections = sum(
            len(r.postprocessed_results) if r.postprocessed_results else 0
            for r in results
        )
