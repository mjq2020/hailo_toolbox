...
from .transforms import (
    ...
    PaddingTransform, CropTransform, InterpolationMethod
)
...
class PreprocessConfig:
    ...
    def _validate_config(self) -> None:
        ...
        if self.target_size is not None:
            if not isinstance(self.target_size, (tuple, list)) or len(self.target_size) != 2:
...
class ImagePreprocessor(BasePreprocessor):
    ...
    def _build_pipeline(self) -> None:
        ...
        if self.config.normalize:
            normalization_transform = NormalizationTransform(
                ...
                dtype=np.dtype(self.config.target_dtype) if self.config.target_dtype else None,
    ...
    def _convert_color_format(self, image: np.ndarray, 
                            from_format: str, to_format: str) -> np.ndarray:
        """
    ...
    def __call__(self, image: np.ndarray) -> np.ndarray:
        ...
        try:
            ...
            raise PreprocessError(
                ...
                details={
                    "input_shape": image.shape if isinstance(image, np.ndarray) else None,
