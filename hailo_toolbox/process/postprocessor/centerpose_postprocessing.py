...
def _topk(scores, K=40):
    ...
    topk_scores, topk_indices = _np_topk(
        np.reshape(scores, (batch_size, category, -1)), K
    )
    ...
    topk_indices = _gather_feat(
        topk_indices.reshape((batch_size, -1, 1)), topk_ind
    ).reshape(batch_size, K)
    topk_ys = _gather_feat(topk_ys.reshape((batch_size, -1, 1)), topk_ind).reshape(
        batch_size, K
    )
    topk_xs = _gather_feat(topk_xs.reshape((batch_size, -1, 1)), topk_ind).reshape(
        batch_size, K
    )
...
def _centerpose_postprocessing(
    ...
    **kwargs,
    ...
    if joint_heatmap is not None:
        keypoints = keypoints.reshape((batch_size, K, num_joints, 2)).transpose(
            (0, 2, 1, 3)
        )  # b x J x K x 2
        ...
        if joint_offset is not None:
            hp_offset = _transpose_and_gather_feat(
                joint_offset, hm_inds.reshape((batch_size, -1))
            )
        ...
        hm_kps = np.tile(
            np.expand_dims(np.stack([hm_xs, hm_ys], axis=-1), 2), (1, 1, K, 1, 1)
        )
        ...
        hm_score = np.expand_dims(
            np.take_along_axis(hm_score, min_ind, axis=2), -1
        )  # b x J x K x 1
        ...
        left = np.tile(
            bboxes[:, :, 0].reshape((batch_size, 1, K, 1)), (1, num_joints, 1, 1)
        )
        top = np.tile(
            bboxes[:, :, 1].reshape((batch_size, 1, K, 1)), (1, num_joints, 1, 1)
        )
        right = np.tile(
            bboxes[:, :, 2].reshape((batch_size, 1, K, 1)), (1, num_joints, 1, 1)
        )
        bottom = np.tile(
            bboxes[:, :, 3].reshape((batch_size, 1, K, 1)), (1, num_joints, 1, 1)
        )
        mask = (
            (hm_kps[..., 0:1] < left)
            + (hm_kps[..., 0:1] > right)
        ...
        keypoints = keypoints.transpose((0, 2, 1, 3)).reshape(
            batch_size, K, num_joints * 2
        )
    ...
    for batch_index in range(batch_size):
        ...
        box = transform_preds(
            box.reshape(-1, 2), center, scale, (out_height, out_width)
        ).reshape(-1, 4)
        keypoint = transform_preds(
            keypoint.reshape(-1, 2), center, scale, (out_height, out_width)
        ).reshape(-1, 34)
