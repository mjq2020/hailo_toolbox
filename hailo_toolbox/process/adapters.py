    ...
    def __init__(self, source_path: str, source_type: str = "file"):
        ...
        if self.is_camera:
            self.cap = cv2.VideoCapture(
                int(source_path) if source_path.isdigit() else source_path
            )
...
class PreprocessorAdapter(PreprocessorInterface):
    ...
    def process(
        self, input_data: DataContainer[np.ndarray]
    ) -> DataContainer[np.ndarray]:
        """
...
class InferenceAdapter(InferenceInterface):
    ...
    def __init__(self, inference_engine, model_info: Dict[str, Any] = None):
        ...
        logger.info(
            f"Initialized InferenceAdapter for {type(inference_engine).__name__}"
        )
    ...
    def process(
        self, input_data: DataContainer[np.ndarray]
    ) -> DataContainer[Dict[str, np.ndarray]]:
        """
        ...
        tensor = (
            ...
            if input_data.preprocessed_data is not None
...
class PostprocessorAdapter(PostprocessorInterface):
    ...
    def __init__(self, task_type: str, config: PostprocessConfig = None):
        ...
        if self.task_type in ["det", "detection"]:
            ...
            self.postprocessor = YOLOv8PosePostprocessor(self.config)
        else:
    ...
    def process(
        self, input_data: DataContainer[Dict[str, np.ndarray]]
    ) -> DataContainer[Any]:
        """
        ...
        raw_outputs = (
            input_data.inference_outputs
            if input_data.inference_outputs is not None
            else input_data.data
        )
    ...
    def _serialize_results(self, results) -> Dict[str, Any]:
        ...
        if hasattr(results, "boxes"):
            ...
            return {
                ...
                "scores": (
                    results.scores.tolist() if results.scores is not None else None
                ),
                "class_ids": (
                    results.class_ids.tolist()
                    if results.class_ids is not None
                    else None
                ),
                "num_detections": len(results),
            }
