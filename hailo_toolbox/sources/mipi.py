...
class MIPICameraSource(BaseSource):
    ...
    def _open_gstreamer(self) -> bool:
        ...
        try:
            ...
            if not self.cap.isOpened():
                raise RuntimeError(
                    f"Failed to open MIPI camera with GStreamer pipeline: {pipeline}"
                )
    ...
    def open(self) -> bool:
        ...
        if self.pipeline_type == "gstreamer" and self.available_backends.get(
            "gstreamer", False
        ):
            success = self._open_gstreamer()
        elif self.pipeline_type == "jetson" and self.available_backends.get(
            "jetson", False
        ):
            success = self._open_jetson()
        elif self.pipeline_type == "picamera2" and self.available_backends.get(
            "picamera2", False
        ):
            success = self._open_picamera2()
        elif self.pipeline_type == "libcamera" and self.available_backends.get(
            "libcamera", False
        ):
        ...
        if not success:
            ...
            if (
                self.available_backends.get("gstreamer", False)
                and self.pipeline_type != "gstreamer"
            ):
            ...
            # Try Jetson-specific API as a fallback
                ...
                self.available_backends.get("jetson", False)
                and self.pipeline_type != "jetson"
            ):
            ...
            # Try picamera2 as a fallback
                ...
                self.available_backends.get("picamera2", False)
                and self.pipeline_type != "picamera2"
            ):
    ...
    def read(self) -> Tuple[bool, Optional[np.ndarray]]:
        ...
        try:
            ...
            # Resize if necessary
            if (
                frame.shape[1] != self.resolution[0]
                or frame.shape[0] != self.resolution[1]
            ):
