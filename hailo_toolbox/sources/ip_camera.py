    ...
    def open(self) -> bool:
        ...
        try:
            ...
            stream_options = {
                ...
                cv2.CAP_PROP_OPEN_TIMEOUT_MSEC: self.timeout
                * 1000,  # Convert to milliseconds
                cv2.CAP_PROP_READ_TIMEOUT_MSEC: self.timeout
                * 1000,  # Convert to milliseconds
            }
            ...
            if not ret:
                raise RuntimeError(
                    f"Could not read initial frame from IP camera: {self.url}"
                )
            ...
            self.buffer_thread = threading.Thread(
                target=self._buffer_frames, daemon=True
            )
        ...
        while not self.stop_event.is_set():
            if self.cap is None or not self.cap.isOpened():
                ...
                if reconnect_count < self.reconnect_attempts:
                    print(
                        f"Reconnecting to IP camera {self.source_id} (attempt {reconnect_count + 1}/{self.reconnect_attempts})..."
                    )
                    reconnect_count += 1
                    ...
                    print(
                        f"Failed to reconnect to IP camera {self.source_id} after {self.reconnect_attempts} attempts"
                    )
                    self.stop_event.set()
                    break
            ...
            time.sleep(
                1.0 / (self.fps * 2)
            )  # Buffer at 2x the playback rate for smoothness
        ...
        info.update(
            {
                ...
                "buffer_length": (
                    len(self.frame_buffer) if hasattr(self, "frame_buffer") else 0
                ),
                "original_resolution": (
