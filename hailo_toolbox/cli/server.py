...
def start_pipeline(
    pipeline_id: str, source_id: str, engine_id: str, config: Dict[str, Any]
) -> bool:
    """
    ...
    with lock:
        ...
        logger.info(
            f"Started pipeline {pipeline_id} with source {source_id} and engine {engine_id}"
        )
        return True
    ...
    with lock:
        ...
        for pipeline_id, pipeline_info in pipelines.items():
            if pipeline_info["engine_id"] == model_id:
                return (
                    jsonify({"error": f"Model is in use by pipeline {pipeline_id}"}),
                    400,
                )
...
def create_source():
    ...
    try:
        ...
        # Return success
        return jsonify(
            {"id": source_id, "type": source_type, "is_open": source.is_open()}
        )
    ...
    with lock:
        ...
        for pipeline_id, pipeline_info in pipelines.items():
            if pipeline_info["source_id"] == source_id:
                return (
                    jsonify({"error": f"Source is in use by pipeline {pipeline_id}"}),
                    400,
                )
            ...
            for model_config in config["models"]:
                try:
                    ...
                    if engine.load():
                        ...
                        logger.error(
                            f"Failed to pre-load model {model_id} from {model_path}"
                        )
            ...
            for pipeline_config in config["pipelines"]:
                try:
                    ...
                    if start_pipeline(pipeline_id, source_id, engine_id, pipe_config):
                        logger.info(
                            f"Pre-started pipeline {pipeline_id} with source {source_id} and engine {engine_id}"
                        )
                    else:
