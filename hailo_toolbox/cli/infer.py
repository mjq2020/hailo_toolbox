...
def load_callback_function(callback_path: str) -> InferenceCallback:
    ...
    try:
        ...
        if len(sig.parameters) < 1:
            raise ValueError(
                f"{callback_path} must accept at least one parameter (InferenceResult)"
            )
...
def visualize_callback(result: InferenceResult) -> None:
    """
    Memory-optimized visualization callback function for common model types.

    This function has been optimized to prevent memory leaks by:
    1. Minimizing array copies and intermediate allocations
    2. Using context managers for proper resource cleanup
    3. Implementing garbage collection hints
    4. Reusing OpenCV windows to prevent window handle leaks
    ...
    # Get the first input image if available with minimal memory allocation
    input_img = None
    try:
        if result.input_data:
            first_input = next(iter(result.input_data.values()))
            ...
            if isinstance(first_input, np.ndarray) and len(first_input.shape) >= 3:
                ...
                # Make a copy for visualization (minimize memory allocation)
                input_img = img.copy()
        ...
        if input_img is not None:
            ...
            if len(first_output.shape) == 2 and first_output.shape[1] > 4:
                ...
                # Draw boxes with memory optimization - work in-place
                h, w = input_img.shape[:2]

                # Process detections in smaller batches to reduce memory usage
                batch_size = min(
                    20, len(detections)
                )  # Process max 20 detections at once
                ...
                for batch_start in range(0, len(detections), batch_size):
                    ...
                    for det in batch_detections:
                        if len(det) >= 5:  # Ensure we have at least x,y,w,h,score
                            ...
                            if (
                                score > 0.3
                            ):  # Confidence threshold (reduced for better performance)
                                ...
                                if (
                                    ...
                                    # Draw the box
                                    cv2.rectangle(
                                        input_img, (x1, y1), (x2, y2), (0, 255, 0), 2
                                    )

                                    # Add score label
                                    label = f"{score:.2f}"
                                    if len(det) >= 6:  # If class ID is available
                                        label = f"C{int(det[5])}:{score:.2f}"  # Shorter label
                ...
                # Show the image (reuse window to prevent handle leaks)
                cv2.namedWindow("Detections", cv2.WINDOW_NORMAL)
                cv2.imshow("Detections", input_img)
                cv2.waitKey(1)
            ...
            # Image classification
                ...
                # Get top 3 classes (reduced from 5 to save memory)
                top_indices = np.argsort(scores)[-3:][::-1]
                top_scores = scores[top_indices]

                # Display image with classification results - work in-place
                h, w = input_img.shape[:2]

                # Add classification results directly on image
                for i, (idx, score) in enumerate(zip(top_indices, top_scores)):
                    label = f"C{idx}:{score:.3f}"  # Shorter format
                    cv2.putText(
                        input_img,
                        label,
                        (10, 30 + i * 20),
                ...
                # Show the image (reuse window)
                cv2.namedWindow("Classification", cv2.WINDOW_NORMAL)
                cv2.imshow("Classification", input_img)
                cv2.waitKey(1)
            ...
            # Segmentation
                ...
                # Resize to match input image with memory optimization
                h, w = input_img.shape[:2]
                ...
                # Create colorized segmentation map with reduced memory usage
                num_classes = int(np.max(seg_map)) + 1

                # Use a smaller color palette to reduce memory
                if num_classes <= 256:
                    ...
                    # Create color map using advanced indexing (more memory efficient)
                    color_map = colors[seg_map.astype(np.int32)]

                    # Create blended output in-place
                    alpha = 0.5
                    cv2.addWeighted(
                        input_img, 1 - alpha, color_map, alpha, 0, dst=input_img
                    )

                    # Clean up intermediate arrays
                    del colors, color_map

                del seg_map  # Clean up segmentation map

                # Show the segmentation (reuse window)
        ...
        # Fallback: show input image if available
        if input_img is not None:
            cv2.namedWindow("Input", cv2.WINDOW_NORMAL)
            cv2.imshow("Input", input_img)
            cv2.waitKey(1)
    ...
    # Print inference information with reduced precision to save memory
    total_time = (
    ...
    print(
        f"Total: {total_time:.1f}ms (pre:{result.preprocessing_time_ms:.1f}, "
        f"infer:{result.inference_time_ms:.1f}, post:{result.postprocessing_time_ms:.1f})"
    )
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Run inference with deep learning models"
    )

    parser.add_argument(
        "model_path", type=str, help="Path to the model file (ONNX format)"
    )
    ...
    parser.add_argument(
        "--visualize", action="store_true", help="Enable basic visualization of results"
    )
    ...
    parser.add_argument(
        "--loop", action="store_true", help="Loop the source if it's a file"
    )

    parser.add_argument(
        "--show-fps", action="store_true", help="Display FPS information"
    )

    parser.add_argument("--verbose", action="store_true", help="Enable verbose logging")

    parser.add_argument(
        "--log-file", type=str, default="infer.log", help="Log file name"
    )
...
def main() -> int:
    ...
    try:
        ...
        try:
            ...
            logger.info(
                "已处理 %d 帧，平均 %.2f FPS", stats["frame_count"], stats["avg_fps"]
            )
