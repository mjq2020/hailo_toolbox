    ...
    def start(self) -> bool:
        ...
        if self.mode == "async":
            ...
            if self.infer_callback:
                ...
                self.visualize_thread = threading.Thread(target=self._process_results, daemon=True)
                self.visualize_thread.start()
                
        elif self.mode == "batch":
            # Start batch processing thread
            self.process_thread = threading.Thread(target=self._process_frames_batch, daemon=True)
            self.process_thread.start()
    ...
    def run_once(self) -> Optional[InferenceResult]:
        ...
        if self.show_fps:
            ...
            if current_time - self.last_fps_report_time > self.fps_report_interval:
                ...
                print(f"FPS: {fps:.2f}, Average: {avg_fps:.2f}, Processing Time: {avg_time:.2f} ms")
