...
def create_mock_detection_output(num_detections: int = 10, num_classes: int = 80) -> Dict[str, np.ndarray]:
    """
    ...
    detection_output[5:, 4:] = np.random.uniform(0.1, 0.3, (num_detections-5, num_classes))  # Low confidence
...
def create_mock_segmentation_output(num_detections: int = 5, num_classes: int = 80) -> Dict[str, np.ndarray]:
    """
...
def create_mock_keypoint_output(num_persons: int = 3, num_keypoints: int = 17) -> Dict[str, np.ndarray]:
    """
    ...
    for i in range(num_persons):
        ...
        for j in range(num_keypoints):
            ...
            keypoint_output[i, kp_idx] = person_x + np.random.uniform(-person_w/2, person_w/2)     # x
            keypoint_output[i, kp_idx+1] = person_y + np.random.uniform(-person_h/2, person_h/2)   # y
...
def example_segmentation_postprocessing():
    ...
    if len(result) > 0:
        ...
        print(f"Mask coverage (% of pixels): {[np.mean(mask > 0) * 100 for mask in result.masks[:3]]}")
...
def example_keypoint_postprocessing():
    ...
    if len(result) > 0:
        ...
        if len(result) > 0:
            ...
            visible_kps = first_person_kps[first_person_kps[:, 2] > config.kp_visibility_threshold]
            print(f"Visible keypoints for first person: {len(visible_kps)}")
            
            # Show some keypoint names and positions
            for i, (kp_name, kp_data) in enumerate(zip(config.keypoint_names, first_person_kps)):
                if kp_data[2] > config.kp_visibility_threshold:  # If visible
                    print(f"  {kp_name}: ({kp_data[0]:.1f}, {kp_data[1]:.1f}) - confidence: {kp_data[2]:.2f}")
