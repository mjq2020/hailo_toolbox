...
class MockInferenceEngine:
    ...
    def infer(self, input_data: dict) -> dict:
        ...
        if self.task_type == "detection":
            ...
            output[0, :, 4:] = np.random.rand(num_detections, 80) * 0.3  # Low confidence for most classes
            output[0, :3, np.random.randint(4, 84, 3)] = 0.9  # High confidence for 3 detections
            ...
            num_detections = 5
            output = np.random.rand(batch_size, num_detections, 84).astype(np.float32)
            masks = np.random.rand(batch_size, num_detections, 160, 160).astype(np.float32)
            return {"output": output, "masks": masks}
        
        elif self.task_type == "keypoint":
            # Keypoint detection output
            num_persons = 3
            output = np.random.rand(batch_size, num_persons, 56).astype(np.float32)  # 4 + 17*3 + 1
            return {"output": output}
...
def debug_with_traceback():
    ...
    try:
        ...
        pipeline_config = PipelineConfig(
            ...
            postprocess_config={'det': True, 'det_conf_threshold': 0.25, 'num_classes': 80},
...
def debug_step_by_step():
    ...
    try:
        ...
        pipeline_config = PipelineConfig(continue_on_error=False)  # Disable error continuation
        pipeline = Pipeline(pipeline_config)
