...
class TestYOLOv8DetPostprocessor(unittest.TestCase):
    ...
    def test_empty_detections(self):
        ...
        detection_output = np.random.rand(num_detections, num_features) * 0.1  # Low scores
        raw_outputs = {"output": detection_output}
    ...
    def test_coordinate_scaling(self):
        ...
        detection_output[:, :4] = [[320, 320, 100, 100],  # Center box
                                   [160, 160, 50, 50],    # Top-left box
                                   [480, 480, 80, 80]]    # Bottom-right box
    ...
    def test_nms_filtering(self):
        ...
        detection_output[:, :4] = [[100, 100, 50, 50],   # High overlap
                                   [110, 110, 50, 50],   # High overlap
                                   [120, 120, 50, 50],   # High overlap
                                   [300, 300, 50, 50]]   # Separate box
        
        # Set high confidence for the first class for all detections
        detection_output[:, 4] = [0.9, 0.8, 0.7, 0.6]  # Decreasing confidence for class 0
...
class TestYOLOv8SegPostprocessor(unittest.TestCase):
    ...
    def test_mask_generation(self):
        ...
        coefficients = np.array([[5.0, 0.0, 0.0, 0.0],  # Select first prototype
                                [0.0, 0.0, 0.0, 5.0]])  # Select fourth prototype
        ...
        boxes = np.array([[100, 100, 300, 300],  # Top-left area
                         [400, 400, 600, 600]])  # Bottom-right area
        
        masks = self.postprocessor._generate_masks(prototypes, coefficients, boxes)
        
        self.assertEqual(masks.shape[0], num_instances)
        self.assertEqual(masks.shape[1:], self.config.input_shape)  # Should match input shape
